...
std::wostream &SplitCodeGen::TRANS_GOTO( RedTransAp *trans, int level )
{
...
std::wostream &SplitCodeGen::PART_MAP()
{
...
{
	out <<
		L"static const int " << START() << L" = " << START_STATE_ID() << L";\n"
		L"\n";

	if ( !noFinal ) {
		out <<
			L"static const int " << FIRST_FINAL() << L" = " << FIRST_FINAL_STATE() << L";\n"
			L"\n";
	}

	if ( !noError ) {
		out <<
			L"static const int " << ERROR() << L" = " << ERROR_STATE() << L";\n"
			L"\n";
	}
...
std::wostream &SplitCodeGen::ALL_PARTITIONS()
{
