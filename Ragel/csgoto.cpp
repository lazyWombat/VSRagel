...
std::wostream &CSharpGotoCodeGen::TO_STATE_ACTION_SWITCH()
{
...
std::wostream &CSharpGotoCodeGen::FROM_STATE_ACTION_SWITCH()
{
	/* Walk the list of functions, printing the cases. */
	for ( GenActionList::Iter act = actionList; act.lte(); act++ ) {
		/* Write out referenced actions. */
...
std::wostream &CSharpGotoCodeGen::ACTION_SWITCH()
{
	/* Walk the list of functions, printing the cases. */
	for ( GenActionList::Iter act = actionList; act.lte(); act++ ) {
		/* Write out referenced actions. */
...
std::wostream &CSharpGotoCodeGen::TO_STATE_ACTIONS()
{
...
std::wostream &CSharpGotoCodeGen::FROM_STATE_ACTIONS()
{
	/* Take one off for the psuedo start state. */
	int numStates = redFsm->stateList.length();
	unsigned int *vals = new unsigned int[numStates];
...
std::wostream &CSharpGotoCodeGen::EOF_ACTIONS()
{
	/* Take one off for the psuedo start state. */
	int numStates = redFsm->stateList.length();
	unsigned int *vals = new unsigned int[numStates];
...
std::wostream &CSharpGotoCodeGen::FINISH_CASES()
{
	for ( RedStateList::Iter st = redFsm->stateList; st.lte(); st++ ) {
...
void CSharpGotoCodeGen::GOTO( wostream &ret, int gotoDest, bool inFinish )
{
	ret << L"{" << vCS() << L" = " << gotoDest << L"; " << 
			CTRL_FLOW() << L"goto _again;}";
}

void CSharpGotoCodeGen::GOTO_EXPR( wostream &ret, GenInlineItem *ilItem, bool inFinish )
{
	ret << L"{" << vCS() << L" = (";
...
void CSharpGotoCodeGen::CURS( wostream &ret, bool inFinish )
{
	ret << L"(_ps)";
}

void CSharpGotoCodeGen::TARGS( wostream &ret, bool inFinish, int targState )
{
	ret << L"(" << vCS() << L")";
}

void CSharpGotoCodeGen::NEXT( wostream &ret, int nextDest, bool inFinish )
{
	ret << vCS() << L" = " << nextDest << L";";
}

void CSharpGotoCodeGen::NEXT_EXPR( wostream &ret, GenInlineItem *ilItem, bool inFinish )
{
	ret << vCS() << L" = (";
	INLINE_LIST( ret, ilItem->children, 0, inFinish );
	ret << L");";
...
void CSharpGotoCodeGen::CALL( wostream &ret, int callDest, int targState, bool inFinish )
{
	if ( prePushExpr != 0 ) {
		ret << L"{";
...
void CSharpGotoCodeGen::CALL_EXPR( wostream &ret, GenInlineItem *ilItem, int targState, bool inFinish )
{
	if ( prePushExpr != 0 ) {
		ret << L"{";
		INLINE_LIST( ret, prePushExpr, 0, false );
...
void CSharpGotoCodeGen::RET( wostream &ret, bool inFinish )
{
...
void CSharpGotoCodeGen::BREAK( wostream &ret, int targState )
{
	outLabelUsed = true;
...
void CSharpGotoCodeGen::writeExec()
{
	...
	{
		out << 
			L"	" << ARRAY_TYPE(redFsm->maxActionLoc) << L" _acts;\n"
			L"	" << ARRAY_TYPE(redFsm->maxActArrItem) << L" _nacts;\n";
	}
	...
	if ( redFsm->anyEofTrans() || redFsm->anyEofActions() ) {
		...
		if ( redFsm->anyEofActions() ) {
			out <<
				L"	" << ARRAY_TYPE(redFsm->maxActionLoc) << L" __acts = " << 
						EA() << L"[" << vCS() << L"];\n"
				L"	" << ARRAY_TYPE(redFsm->maxActArrItem) << L" __nacts = " <<
					A() << L"[__acts++];\n"
				L"	while ( __nacts-- > 0 ) {\n"
				L"		switch ( " << A() << L"[__acts++] ) {\n";
				EOF_ACTION_SWITCH();
